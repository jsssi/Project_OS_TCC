...
export  class OrderService{
  ...
  constructor(HttpClient:HttpClient , private UserService:UserService, private authService: AuthService){
  ...
  GerarOdemDeSerice(cpf:string ) {

    this.UserService.getClientByCpf(cpf , this.token).subscribe((cliente) => {
      const doc = new jsPDF();
      ...
      const orderDetails = [
        ['Descrição', cliente.order_id.description || 'Nao informado'],
        ['Material', cliente.order_id.material || 'Nao informado'],
        ['Data de Emissão', cliente.order_id.estimatedTime || 'Nao informado'],
        ['Data Estimada', cliente.order_id.estimatedTime || 'Nao informado'],
      ];
      doc.autoTable({
        startY: finalY2 + 10,
        head: [['Campo', 'Detalhe']],
