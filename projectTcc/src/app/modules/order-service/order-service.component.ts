        ...
        switchMap((responsePhone) => {
          ...
          return this.OrderService.setOrderService(order, this.token).pipe(
            tap((responseOrder) => {
              ...
              client.orderId = responseOrder.id; // Atribuindo o id da ordem ao cliente
            }),
            switchMap(() =>
              this.userService.CreateUser(client, responsePhone, this.token, client.orderId)
            ),
            catchError((error) => {
      ...
      .subscribe({
        next: (responseUser) => {
          if (responseUser) {
            ...
            this.OrderService.GerarOdemDeSerice(client.cpf, this.token); // Gera o PDF da ordem de serviço
          }
        },
        error: (error) => {
